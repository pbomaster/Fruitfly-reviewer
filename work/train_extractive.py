...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--out',default='outputs/extractive-v5')
    p.add_argument('--steps',type=int,default=2000); p.add_argument('--resume',action='store_true')
    ...
    tasks=ExtractiveTasks(); m=ExtractivePointer(); initial,ii=Checkpoints('outputs/query-pointer-v4/best').load()
    missing,unexpected=m.load_state_dict(initial['model'],strict=False)
    assert set(missing)=={'sentence_key.weight','boundary.weight'} and not unexpected
    opt=torch.optim.AdamW(m.parameters(),lr=.0003,weight_decay=.01)
    config={'version':5,'batch':8,'lr':.0003,'seed':55123,'init':ii,'corpus_sha256':sha256(PATH),
            'code':{f:sha256('work/'+f) for f in ['train_extractive.py','extractive_tasks.py','extractive_pointer.py','query_pointer.py','graph_pointer.py','connectome_memory.py','resumable_checkpoint.py']},
            'scope':'Weakly supervised extractive contribution-plus-limitation paragraph; not expert review labels.','target_pdf_used':False}
