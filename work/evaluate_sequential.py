...
from train_sequential import generate,loss_batch
...
def main():
    ...
    out=Path('outputs/sequential-v6'); m=SequentialPointer(); saved,info=Checkpoints(out/'best').load()
    m.load_state_dict(saved['model']); tasks=ExtractiveTasks(); start=time.monotonic()
    examples=[tasks.example(960000000+i,'test') for i in range(32)]
