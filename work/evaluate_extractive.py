...
from train_extractive import generate,loss_batch
...
def main():
    ...
    out=Path('outputs/extractive-v5'); m=ExtractivePointer(); saved,info=Checkpoints(out/'best').load()
    m.load_state_dict(saved['model']); tasks=ExtractiveTasks(); start=time.monotonic()
    examples=[tasks.example(950000000+i,'test') for i in range(32)]
