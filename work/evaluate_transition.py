...
from train_transition import generate,loss_batch
...
def main():
    ...
    out=Path('outputs/transition-v7'); m=SequentialPointer(); saved,info=Checkpoints(out/'best').load()
    m.load_state_dict(saved['model']); tasks=ExtractiveTasks(); start=time.monotonic()
    examples=[tasks.example(970000000+i,'test') for i in range(32)]
