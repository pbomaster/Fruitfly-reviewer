...
from section_tasks import SectionTasks,PATH
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--out',default='outputs/sections-v9')
    ...
    tasks=SectionTasks(); m=CoveragePointer(); initial,ii=Checkpoints('outputs/coverage-v8/best').load()
    ...
    config={'version':9,'batch':8,'lr':.0003,'seed':55123,'init':ii,'corpus_sha256':sha256(PATH),
            'code':{f:sha256('work/'+f) for f in ['train_sections.py','coverage_pointer.py','section_tasks.py','extractive_tasks.py','extractive_pointer.py','query_pointer.py','graph_pointer.py','connectome_memory.py','resumable_checkpoint.py']},
            'scope':'Publisher abstract/body markers, refined limitations, 24 source sentences. External coverage bookkeeping feeds graph sensory neurons; learned revisit penalty. Transition-weighted token/alignment loss plus supervised copy/EOS gate. Same v6 architecture and corpus. Weakly supervised extractive contribution-plus-limitation paragraph; not expert review labels.','target_pdf_used':False}
