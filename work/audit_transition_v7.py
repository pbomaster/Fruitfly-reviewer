...
@torch.no_grad()
def main():
    torch.set_num_threads(4); m=SequentialPointer().eval()
    saved,info=Checkpoints('outputs/transition-v7/best').load(); m.load_state_dict(saved['model'])
    ...
    atomic_json('outputs/transition-v7/transition-audit-raw.json',rows)
    atomic_json('outputs/transition-v7/transition-audit.json',summary); print(json.dumps(summary,indent=2))
