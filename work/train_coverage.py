...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--out',default='outputs/coverage-v8')
    ...
    tasks=ExtractiveTasks(); m=CoveragePointer(); initial,ii=Checkpoints('outputs/transition-v7/best').load()
    missing,unexpected=m.load_state_dict(initial['model'],strict=False)
    assert set(missing)=={'coverage_input.weight','coverage_penalty'} and not unexpected
    opt=torch.optim.AdamW(m.parameters(),lr=.0003,weight_decay=.01)
    config={'version':8,'batch':8,'lr':.0003,'seed':55123,'init':ii,'corpus_sha256':sha256(PATH),
            'code':{f:sha256('work/'+f) for f in ['train_coverage.py','coverage_pointer.py','extractive_tasks.py','extractive_pointer.py','query_pointer.py','graph_pointer.py','connectome_memory.py','resumable_checkpoint.py']},
            'scope':'External coverage bookkeeping feeds graph sensory neurons; learned revisit penalty. Transition-weighted token/alignment loss plus supervised copy/EOS gate. Same v6 architecture and corpus. Weakly supervised extractive contribution-plus-limitation paragraph; not expert review labels.','target_pdf_used':False}
